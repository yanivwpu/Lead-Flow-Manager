...
type Dashboard = { config: { configured: boolean; missing: string[] }; lastSuccessfulSync: string | null; period: { clicks:number; impressions:number; ctr:number; position:number; previous:{clicks:number;impressions:number;ctr:number;position:number} }; topQueries:Array<{name:string;clicks:number;impressions:number}>; topPages:Array<{name:string;clicks:number;impressions:number}>; opportunities:Array<{type:string;priority:number;query:string;page?:string;evidence:Record<string,number|string>}> };
...
export function AdminSeoIntelligenceTab({ enabled }: { enabled: boolean }) {
  ...
  return <div className="space-y-5" data-testid="seo-intelligence">
    ...
    {sync.isSuccess && <div className="text-emerald-700"><CheckCircle2 className="inline h-4 w-4 mr-2" />Synchronization completed.</div>}
    {sync.isError && <div className="text-red-700"><AlertCircle className="inline h-4 w-4 mr-2" />{sync.error.message}</div>}
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">{[["Clicks",d.period.clicks,delta(d.period.clicks,d.period.previous.clicks)],["Impressions",d.period.impressions,delta(d.period.impressions,d.period.previous.impressions)],["CTR",`${(d.period.ctr*100).toFixed(2)}%`,delta(d.period.ctr,d.period.previous.ctr)],["Avg. position",d.period.position.toFixed(1),delta(d.period.previous.position,d.period.position)]].map(([label,value,change])=><div className="bg-white border rounded-xl p-4" key={String(label)}><p className="text-xs text-gray-500">{label} · 28 days</p><p className="text-2xl font-semibold">{value}</p><p className="text-xs text-gray-500">vs previous: {change}</p></div>)}</div>
    <div className="grid lg:grid-cols-2 gap-4">{[["Top queries",d.topQueries],["Top pages",d.topPages]].map(([title, rows])=><div className="bg-white border rounded-xl p-4" key={String(title)}><h3 className="font-semibold mb-3">{title as string}</h3>{(rows as Dashboard["topQueries"]).map(r=><div className="flex justify-between gap-3 py-2 border-t text-sm" key={r.name}><span className="truncate">{r.name}</span><span className="whitespace-nowrap">{r.clicks} clicks · {r.impressions} imp.</span></div>)}</div>)}</div>
