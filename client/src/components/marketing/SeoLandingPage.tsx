...
export function SeoLandingPage({ config }: Props) {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <header className="border-b border-gray-100 bg-gradient-to-b from-gray-50 to-white px-4 py-14 md:px-6 md:py-20">
        <div className="mx-auto max-w-3xl">
          ...
          <p className="mx-auto mt-6 max-w-2xl text-lg leading-relaxed text-gray-600">{config.heroIntro}</p>
          <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
            ...
            <Link href="/user-guide">
              <a className="text-sm font-medium text-brand-green hover:underline">Read the Help Center guide</a>
            </Link>
          </div>
          </div>
        </div>
      ...
      <main className="mx-auto max-w-3xl px-4 py-12 md:px-6 md:py-16">
        ...
        {config.sections.map((section) => (
          <section key={section.id} id={section.id} className="mb-14 scroll-mt-24">
            <h2 className="font-display mb-4 text-2xl font-bold text-gray-900 md:text-3xl">{section.title}</h2>
            ...
            {section.image ? (
              <figure className="mt-6 overflow-hidden rounded-xl border border-gray-100 bg-gray-50 shadow-sm">
                <img
        ...
        {config.faqs.length > 0 ? (
          <section id="faq" className="mb-14 scroll-mt-24">
            <h2 className="font-display mb-6 text-2xl font-bold text-gray-900 md:text-3xl">Frequently asked questions</h2>
            <dl className="space-y-6">
              {config.faqs.map((f) => (
                <div key={f.question} className="rounded-xl border border-gray-100 p-5">
