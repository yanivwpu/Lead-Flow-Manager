/**
 * Inbound processing normalized state contract — all launch channels.
 * Run: ALLOW_DB_TEST_WRITES=1 npx tsx tests/inbound-processing-contract.test.ts
 *      (or set TEST_DATABASE_URL)
 */
import assert from "node:assert/strict";
import { prepareDbTestEnvironment, teardownTestUser } from "./helpers/dbTestGuard.js";

prepareDbTestEnvironment("inbound-processing-contract.test.ts");

const { channelService } = await import("../server/channelService");
const { scheduleW2FollowUpTimers } = await import("../server/automationTimerHandlers");
...
async function main() {
  ...
  try {
    ...
    await teardownTestUser(userId, "inbound-processing-contract.test.ts");
  }
}
