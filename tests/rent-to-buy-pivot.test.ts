...
assert(
  beachPatch.geoPreferences?.value?.some((a) => /close to beach/i.test(a)),
  "beach proximity captured as geo preference",
);
...
assert(
  afterBeach.geoPreferences?.value?.some((a) => /close to beach/i.test(a)),
  "beach geo preference merged",
);
