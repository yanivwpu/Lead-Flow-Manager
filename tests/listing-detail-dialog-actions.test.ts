...
const blockedShare = getShareListingButtonState({
  ...
  directShare: {
    allowed: false,
    blockedReason: "Listing is not compliance-eligible for public publishing",
  },
  directShareLoaded: true,
});
assert(blockedShare.show && !blockedShare.enabled, "share listing shown but disabled when blocked");
assert(
  blockedShare.blockedReason?.includes("compliance-eligible"),
  "blocked reason surfaced for disabled share button",
);
