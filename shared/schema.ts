...
export const users = pgTable("users", {
  ...
  widgetSettings: jsonb("widget_settings").default(sql`'{"enabled":false,"color":"#10b981","welcomeMessage":"Hi! How can we help you today?","position":"right","showOnMobile":true,"showOnDesktop":true,"triggerType":"always","triggerDelaySeconds":5,"triggerScrollPercent":50,"pageRules":[],"allowedOrigins":[],"allowAnyOrigin":false}'::jsonb`),
...
/** Phase 2B research/approval records. They intentionally expose no publishing primitive. */
export const seoAnalysisRuns = pgTable("seo_analysis_runs", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`), propertyId: text("property_id").notNull(), status: text("status").notNull().default("running"), trigger: text("trigger").notNull().default("manual"),
  leaseToken: text("lease_token").notNull(), leaseExpiresAt: timestamp("lease_expires_at").notNull(), diagnostics: jsonb("diagnostics").notNull().default({}), failureCategory: text("failure_category"), startedAt: timestamp("started_at").notNull().defaultNow(), completedAt: timestamp("completed_at"),
}, t => ({ propertyStartedIdx: index("seo_analysis_runs_property_started_idx").on(t.propertyId, t.startedAt) }));
export const seoOpportunities = pgTable("seo_opportunities", {
 id: varchar("id").primaryKey().default(sql`gen_random_uuid()`), propertyId: text("property_id").notNull(), clusterKey: varchar("cluster_key", {length:64}).notNull(), targetPage:text("target_page").notNull(), queryCluster:jsonb("query_cluster").notNull(), opportunityType:text("opportunity_type").notNull(), currentMetrics:jsonb("current_metrics").notNull(), previousMetrics:jsonb("previous_metrics").notNull(), priorityScore:doublePrecision("priority_score").notNull(), confidenceScore:doublePrecision("confidence_score").notNull(), estimatedUpside:doublePrecision("estimated_upside").notNull(), reason:text("reason").notNull(), evidence:jsonb("evidence").notNull(), detectedAt:timestamp("detected_at").notNull().defaultNow(), refreshedAt:timestamp("refreshed_at").notNull().defaultNow(),
}, t => ({ clusterIdx: index("seo_opportunities_property_cluster_idx").on(t.propertyId,t.clusterKey,t.detectedAt) }));
export const seoPageSnapshots = pgTable("seo_page_snapshots", { id:varchar("id").primaryKey().default(sql`gen_random_uuid()`), propertyId:text("property_id").notNull(), pageUrl:text("page_url").notNull(), contentFingerprint:varchar("content_fingerprint",{length:64}).notNull(), title:text("title"), metaDescription:text("meta_description"), canonicalUrl:text("canonical_url"), headings:jsonb("headings").notNull().default([]), sections:jsonb("sections").notNull().default([]), structuredData:jsonb("structured_data").notNull().default([]), internalLinks:jsonb("internal_links").notNull().default([]), deficiencies:jsonb("deficiencies").notNull().default([]), metrics:jsonb("metrics").notNull().default({}), capturedAt:timestamp("captured_at").notNull().defaultNow() });
export const seoActions = pgTable("seo_actions", {
 id:varchar("id").primaryKey().default(sql`gen_random_uuid()`), propertyId:text("property_id").notNull(), opportunityId:varchar("opportunity_id").notNull(), idempotencyKey:varchar("idempotency_key",{length:64}).notNull(), targetPage:text("target_page").notNull(), queryCluster:jsonb("query_cluster").notNull(), actionType:text("action_type").notNull(), status:text("status").notNull().default("detected"), risk:text("risk").notNull(), confidence:doublePrecision("confidence").notNull(), expectedBenefit:text("expected_benefit").notNull(), originalContentFingerprint:varchar("original_content_fingerprint",{length:64}).notNull(), currentVersion:integer("current_version").notNull().default(1), approvedBy:varchar("approved_by"), approvedAt:timestamp("approved_at"), rejectedBy:varchar("rejected_by"), rejectedAt:timestamp("rejected_at"), rejectionReason:text("rejection_reason"), staleAt:timestamp("stale_at"), refreshLeaseToken:text("refresh_lease_token"), refreshStartedAt:timestamp("refresh_started_at"), refreshLeaseExpiresAt:timestamp("refresh_lease_expires_at"), refreshFailureCategory:text("refresh_failure_category"), refreshReturnStatus:text("refresh_return_status"), staleCheckedAt:timestamp("stale_checked_at"), staleCheckRetryAt:timestamp("stale_check_retry_at"), createdAt:timestamp("created_at").notNull().defaultNow(), updatedAt:timestamp("updated_at").notNull().defaultNow(),
}, t => ({ propertyStatusIdx:index("seo_actions_property_status_idx").on(t.propertyId,t.status,t.updatedAt) }));
export const seoActionVersions = pgTable("seo_action_versions", { id:varchar("id").primaryKey().default(sql`gen_random_uuid()`), actionId:varchar("action_id").notNull(), version:integer("version").notNull(), proposal:jsonb("proposal").notNull(), aiProvider:text("ai_provider").notNull(), aiModel:text("ai_model").notNull(), promptVersion:text("prompt_version").notNull(), pageSnapshotId:varchar("page_snapshot_id"), evidenceSnapshot:jsonb("evidence_snapshot").notNull(), createdAt:timestamp("created_at").notNull().defaultNow() });
export const seoActionEvents = pgTable("seo_action_events", { id:bigint("id",{mode:"number"}).primaryKey().generatedAlwaysAsIdentity(), actionId:varchar("action_id").notNull(), fromStatus:text("from_status"), toStatus:text("to_status").notNull(), actorId:varchar("actor_id"), reason:text("reason"), safeMetadata:jsonb("safe_metadata").notNull().default({}), createdAt:timestamp("created_at").notNull().defaultNow() });
