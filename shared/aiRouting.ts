...
const INFO_SEEKING_RE =
  /\b(?:learn\s+more|tell\s+me\s+more|more\s+information|more\s+info|more\s+details|how\s+does\s+(?:it|this|your|the)\s+work|how\s+do\s+i\b|how\s+can\s+i\b|how\s+do\s+(?:you|your)|how\s+much\b|how\s+many\b|what(?:'s|\s+is|\s+are)\b|what\s+(?:is|are)\s+(?:your|the)|(?:what(?:'s|\s+are)\s+)?the\s+benefits?|benefits?\b|pricing\b|prices?\b|costs?\b|fees?\b|rates?\b|per\s+month|monthly\b|plans?\b|packages?\b|features?\s*(?:&|and)\s*pricing|find(?:ing)?\s+my\s+solution|interested\s+in|curious\s+about|want\s+to\s+(?:know|learn|understand)|looking\s+to\s+learn|about\s+your\s+(?:product|service|platform|automation|software|tool|features?)|(?:your|the)\s+features?\b|what\s+can\s+(?:it|you|this)\s+do|can\s+you\s+(?:help|tell|explain)\b|do\s+you\s+(?:offer|have|support)|join\s+(?:the\s+)?(?:directory|platform)|list\s+my\b|advertis(?:e|ing)\b)\b/i;
