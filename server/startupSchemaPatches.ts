...
`.trim(),
  ...
  {
    tag: "0087_ghl_marketplace_billing",
    sql: `
...
`.trim(),
  ...
  {
    tag: "0089_webchat_tenant_isolation",
    sql: `
...
CREATE UNIQUE INDEX IF NOT EXISTS users_widget_public_id_uidx ON users (widget_public_id) WHERE widget_public_id IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS users_telegram_webhook_public_id_uidx ON users (telegram_webhook_public_id) WHERE telegram_webhook_public_id IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS users_tiktok_lead_public_id_uidx ON users (tiktok_lead_public_id) WHERE tiktok_lead_public_id IS NOT NULL;
...
UPDATE flow_jobs
SET user_id = COALESCE(user_id, NULLIF(payload->>'userId', ''), (SELECT cf.user_id FROM chatbot_flows cf WHERE cf.id = flow_jobs.flow_id LIMIT 1))
...
`.trim(),
  ...
  // 0093 matches migrations/0093_seo_intelligence.sql. These tables must exist
  // before admin routes are served or the scheduled read-only import can run.
  {
    tag: "0093_seo_intelligence",
    sql: [
...
export async function applyStartupSchemaPatches(): Promise<{
  ...
  seoIntelligencePatchOk: boolean;
}> {
