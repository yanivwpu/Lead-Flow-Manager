...
export type QueryLanguageAssessment={status:"compatible"|"uncertain"|"incompatible";clean:string;pageLanguage:RecommendationLanguage};
...
/** Script compatibility is evidence, not an English-word whitelist. No query is rewritten as a generic intent. */
export function assessQueryLanguage(query:string,targetPage:string):QueryLanguageAssessment{const pageLanguage=recommendationLanguageForPage(targetPage),clean=query.replace(/[\u0000-\u001f\u007f]+/gu," ").replace(/\s+/gu," ").trim(),hasExpected=SCRIPT_PATTERN[pageLanguage].test(clean),hasForeign=FOREIGN_SCRIPT.test(clean),compatible=hasExpected&&(!hasForeign||(pageLanguage==="he"&&/\p{Script=Hebrew}/u.test(clean)));return{status:compatible?"compatible":clean&&!hasExpected&&!hasForeign?"uncertain":"incompatible",clean,pageLanguage};}
...
export function proposedMetaDescriptionForQuery(query: string,targetPage="/",firstPartyIntent?:string) {
  const assessment=assessQueryLanguage(query,targetPage),intent=assessment.status==="compatible"?assessment.clean:assessment.status==="incompatible"?safeFirstPartyIntent(firstPartyIntent,targetPage):undefined;
  if(!intent)throw Object.assign(new Error(assessment.status==="uncertain"?"Query language is uncertain":"No language-compatible first-party intent is available"),{category:assessment.status==="uncertain"?"QUERY_LANGUAGE_UNCERTAIN":"QUERY_LANGUAGE_MISMATCH"});
